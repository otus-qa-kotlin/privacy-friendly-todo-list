...
object Helper {
    @JvmField
    val DATE_FORMAT: CharSequence = "dd.MM.yyyy"

    @JvmStatic
    fun getDate(time: Long): String {
        val calendar = Calendar.getInstance()
        calendar.timeInMillis = TimeUnit.SECONDS.toMillis(time)
    ...
    fun getDeadlineColor(context: Context?, color: DeadlineColors?): Int {
        return when (color) {
            ...
            DeadlineColors.ORANGE -> ContextCompat.getColor(
                context!!,
                R.color.deadline_orange
            )

            else -> throw IllegalArgumentException("Deadline color not defined.")

        }
    }

    @JvmStatic
    fun priority2String(context: Context, prio: TodoTask.Priority?): String {
        return when (prio) {
            TodoTask.Priority.HIGH -> context.resources.getString(
                R.string.high_priority
            )

            TodoTask.Priority.MEDIUM -> context.resources.getString(
                R.string.medium_priority
            )

            TodoTask.Priority.LOW -> context.resources.getString(
    ...
    fun getMenuHeader(context: Context?, title: String?): TextView {
        ...
        blueBackground.layoutParams = LinearLayout.LayoutParams(
