...
class TodoListWidget : AppWidgetProvider() {
    ...
    override fun onDeleted(context: Context, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            TodoListWidgetConfigureActivity.Companion.deleteTitlePref(context, appWidgetId)
        }
    }
    ...
    fun refreshWidget(context: Context?, appWidgetId: Int): PendingIntent {
        ...
        return PendingIntent.getBroadcast(context, 0, update, PendingIntent.FLAG_UPDATE_CURRENT)
    }

    fun updateHelper(context: Context) {
    ...
    fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
        if (views == null) views = RemoteViews(context.packageName, R.layout.todo_list_widget)
    ...
    companion object {
        var listChosen: String? = null
        fun getListName(context: Context?, AppWidgetId: Int) {
            listChosen =
                TodoListWidgetConfigureActivity.Companion.loadTitlePref(context, AppWidgetId)
        }
    }
}