 ...
 *
 */
class CalendarGridAdapter(context: Context?, resource: Int) : ArrayAdapter<Date?>(
    context!!, resource, ArrayList()
) {
    ...
    override fun getView(position: Int, view: View?, parent: ViewGroup): View {
        ...
        posCal.time = dateAtPos
        if (v == null) {
            dayViewHolder = CalendarDayViewHolder()
            v = inflater.inflate(R.layout.calendar_day, parent, false)
            dayViewHolder.dayText =
                v.findViewById<View>(R.id.tv_calendar_day_content) as TextView
            oldColors = dayViewHolder.dayText!!.textColors
            v.tag = dayViewHolder
        } else {
            dayViewHolder = v.tag as CalendarDayViewHolder
        }
        ...
        if (posCal[Calendar.MONTH] != currentMonth) {
            ...
            dayViewHolder.dayText!!.setTextColor(
        ...
        if (tasksToday != null) {
            ...
            for (t in tasksToday) {
                if (!t.done) border = ContextCompat.getDrawable(context, R.drawable.border_blue)
            }
